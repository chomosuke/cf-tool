...
func getLatest() (version, note, ptime, url string, size uint, err error) {
	...
	resp, err := http.Get("https://api.github.com/repos/chomosuke/cf-tool/releases/latest")
	if err != nil {
		return
	}
	...
	url = fmt.Sprintf("https://github.com/chomosuke/cf-tool/releases/download/%v/cf_%v_%v_%v.zip", version, version, goos, arch)
